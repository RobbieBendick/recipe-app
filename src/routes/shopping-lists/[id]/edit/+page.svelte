<script lang="ts">
	...
	import { aggregateIngredientLines, mergeIngredientLists, subtractIngredientLists } from '$lib/ingredients';
	...
	let title = $state('');
	let itemsText = $state('');
	/** Items already on the list when editing started (kept when recipes are added). */
	let seedItems = $state<string[]>([]);
	...
	onMount(async () => {
		try {
			...
			if (list) {
				title = list.title;
				const lines = list.items.map((item) => item.text);
				const savedCounts: Record<string, number> = {};
				for (const [recipeId, count] of Object.entries(list.recipeCounts ?? {})) {
					if (count > 0) savedCounts[recipeId] = count;
				}
				recipeCounts = savedCounts;

				const fromRecipes = loadedRecipes
					.map((recipe) => {
						const servings = savedCounts[recipe.id] ?? 0;
						if (servings <= 0) return [];
						return aggregateIngredientLines(recipe.ingredients, servings);
					})
					.filter((part) => part.length > 0);

				seedItems = subtractIngredientLists(lines, mergeIngredientLists(fromRecipes));
				itemsText = listToLines(lines);
			}
		} catch (e) {
	...
	function countFor(recipeId: string): number {
		return recipeCounts[recipeId] ?? 0;
	}
	...
	function rebuildItems(counts: Record<string, number>) {
		...
		itemsText = mergeIngredientLists([seedItems, ...fromRecipes]).join('\n');
	}
...
<main class="page">
	...
	{:else}
		<header class="intro">
			...
			<h1>Edit list</h1>
			<p class="lede">Drag recipes onto the list to add their ingredients.</p>
		</header>
			...
			<form class="form" onsubmit={save}>
				...
				>
					<div class="drop__hint">
						...
						{:else if totalAdds > 0}
							{totalAdds} recipe{totalAdds === 1 ? '' : 's'} on this list
						{:else}
							Drop recipes here
						{/if}
					</div>
